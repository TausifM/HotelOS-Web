...
import {
  ...
  Bell, TrendingUp, Phone, AlertCircle, X, Sparkles,
  Megaphone, CalendarClock, RotateCw, Users,
} from 'lucide-react';
...
interface Notification {
  ...
  reservationId?: { _id: string; bookingRef?: string; roomNumber?: string } | null;
  ...
  createdAt: string;
  sentAt?: string;
  externalId?: string;       // ← was `messageId` in old FE
  failureReason?: string;    // ← was `error` in old FE
  retryCount?: number;
}

interface StatsResponse {
  byStatus:  Array<{ _id: { channel: string; status: string }; count: number }>;
...
/* ──────────────────────────────────────────────────────────────────────────────
   PALETTE — orange & pink gradient theme
────────────────────────────────────────────────────────────────────────────── */
const MELON = '#F97316';
const CORAL = '#F43F5E';
const PINK  = '#EC4899';
...
const TRIGGER_LABELS: Record<string, string> = {
  ...
  pre_arrival_2h: 'Pre-arrival 2h',
  checkin_done: 'Check-in',
  checkout_reminder: 'Checkout Reminder',
  checkout_done: 'Post-checkout',
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: highlight ? 'rgba(255,255,255,0.22)' : bg,
      ...
      <p
        ...
        style={{ color: highlight ? '#fff' : '#1C0A02' }}
      >
        {typeof value === 'number' ? value.toLocaleString() : value}
      </p>
      <p
        className="mt-0.5 text-xs font-medium"
        style={{ color: highlight ? 'rgba(255,255,255,0.85)' : '#A8836C' }}
      >
        {label}
      </p>
    </motion.div>
  ...
  const styles =
    variant === 'gradient'
      ...
      ? {
          ...
          color: CORAL,
          border: `1.5px solid ${MELON}33`,
        }
      : {
          background: '#fff',
          color: '#6B4535',
          border: `1.5px solid ${MELON}30`,
          boxShadow: `0 2px 10px ${MELON}08`,
        };
...
/* ──────────────────────────────────────────────────────────────────────────────
   SEND MODAL — single guest, with optional schedule
────────────────────────────────────────────────────────────────────────────── */
function SendModal({
  open,
  onClose,
 ...
 function openWhatsAppLink(phone: string, text: string) {
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(schedule ? 'Message scheduled' : 'Message sent');
      onSent();
      handleClose();
    },
    onError: (e: any) =>
  ...
  const vars: Record<string, string> = {
    ...
    tenantName: tenant?.name || tenant?.hotelName || '{tenantName}',
    ...
    <Modal open={open} onClose={handleClose} title="Send WhatsApp Message" size="lg">
      <div className="space-y-4">
        {step === 'guest' && (
          <>
            <div>
              <p className="mb-2 text-sm font-semibold" style={{ color: '#6B4535' }}>
                Search guest
              </p>
              <div
                className="relative flex items-center rounded-xl px-3 py-2.5"
                style={{
                  background: '#fff',
                  border: `1.5px solid ${MELON}30`,
                ...
                {guests.map((g: any) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold" style={{ color: '#1C0A02' }}>
                        {g.firstName} {g.lastName}
                      </p>
                      <p className="truncate text-xs" style={{ color: '#A8836C' }}>
                        {g.whatsappNumber || g.phone}
                      </p>
                    </div>
            ...
            {guestSearch.length >= 2 && !guestsLoading && !guests?.length && (
              <p className="py-4 text-center text-sm" style={{ color: '#A8836C' }}>
                No guests found
              </p>
            )}
          </>
        ...
        {step === 'message' && selectedGuest && (
          <>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold" style={{ color: '#1C0A02' }}>
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                <p className="text-sm font-semibold" style={{ color: '#6B4535' }}>
                  Message
                </p>
                <span
                  className="text-xs font-semibold"
                  style={{ color: message.length > 900 ? CORAL : '#A8836C' }}
                >
                  {message.length}/1000
                </span>
              </div>
              <textarea
                ...
                placeholder="Type your message... (supports *bold*, _italic_)"
              ...
              <p className="mt-1 text-xs" style={{ color: '#A8836C' }}>
                Tip: Use *bold*, _italic_ for WhatsApp formatting. Placeholders like {'{bookingRef}'}, {'{roomNumber}'} are filled at send time.
              </p>
            </div>
            ...
            <div className="flex gap-2 pt-1">
              <PrimaryBtn variant="outline" onClick={() => setStep('guest')}>
                ← Back
              </PrimaryBtn>
...
function BroadcastModal({
  ...
  const broadcastMutation = useMutation({
    mutationFn: () =>
      api.post('/api/notifications/broadcast', { targetGroup, message }),
    onSuccess: (r) => {
      const d = r.data?.data || {};
    ...
    <Modal open={open} onClose={handleClose} title="Broadcast Message" size="lg">
      <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {BROADCAST_GROUPS.map((g) => {
              ...
              return (
                ...
                >
                  <p className="text-sm font-bold" style={{ color: active ? CORAL : '#1C0A02' }}>
                    {g.label}
                  </p>
                  <p className="text-xs" style={{ color: '#A8836C' }}>
                    {g.desc}
                  </p>
                </button>
              );
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs" style={{ color: '#A8836C' }}>
            {'{name}'} will be replaced with each guest's first name.
          </p>
        </div>
        ...
        <div
          ...
          style={{
            background: '#FEF2F2',
            border: `1.5px solid ${CORAL}33`,
            color: '#991B1B',
          }}
        >
          <AlertCircle className="mt-0.5 h-4 w-4 flex-shrink-0" />
          <p>
            This sends a WhatsApp message to every matching guest immediately. Rate-limited at ~12 messages/sec.
          </p>
        </div>

        <div className="flex gap-2">
          <PrimaryBtn variant="outline" onClick={handleClose}>
            Cancel
          </PrimaryBtn>
          <div className="flex-1">
            <PrimaryBtn
...
export default function NotificationsPage() {
  ...
  const { staff, tenant, isAuthenticated, hasPermission, hasRole } = useAuth();
  ...
  const { data: statsRaw } = useQuery<StatsResponse>({
    ...
    queryFn: () =>
      api.get('/api/notifications/stats').then((r) => r.data.data),
  ...
  const { data: templates = [] } = useQuery<Template[]>({
    ...
    queryFn: () =>
      api.get('/api/notifications/templates').then((r) => r.data.data || []),
    staleTime: 5 * 60_000,
  });

  // ── Derived stats from real backend shape ────────────────────────────────
  ...
  const filtered = search
    ? docs.filter(
        (n) =>
          `${n.guestId?.firstName || ''} ${n.guestId?.lastName || ''}`
            .toLowerCase()
            .includes(search.toLowerCase()) ||
          n.to.includes(search) ||
          ...
          <div className="relative flex flex-wrap items-start justify-between gap-3">
            <div className="flex items-center gap-4">
              <div
                ...
                style={{
                  background: GRADIENT_PRIMARY,
                  boxShadow: `0 8px 24px ${MELON}55`,
                }}
              >
                <Bell className="h-5 w-5" />
              </div>
              <div>
                <h1 className="text-2xl font-bold tracking-tight" style={{ color: '#1C0A02' }}>
                  Notifications
                </h1>
                <p className="mt-0.5 text-sm" style={{ color: '#6B4535' }}>
                  WhatsApp &amp; messaging logs ·{' '}
                  <span className="font-semibold" style={{ color: MELON }}>
            ...
            <div className="flex flex-wrap gap-2">
              <PrimaryBtn variant="outline" onClick={() => refetch()} icon={
                <RefreshCw className={cn('h-3.5 w-3.5', isFetching && 'animate-spin')} style={{ color: MELON }} />
              }>
                Refresh
              </PrimaryBtn>
...
{(hasRole('owner', 'manager') || hasPermission('notifications.broadcast')) && (
  <PrimaryBtn
    variant="soft"
  ...
  >
    Broadcast
        ...
        >
          ...
          <StatTile label="Delivery Rate" value={`${deliveryRate}%`} icon={TrendingUp} tint="#fff"    bg="" highlight />
        </motion.div>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold" style={{ color: active ? CORAL : '#1C0A02' }}>
                      {cfg.label}
                    </p>
                    <p className="text-xs" style={{ color: '#A8836C' }}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div
            className="flex min-w-48 flex-1 items-center gap-2 rounded-xl px-3 py-2"
            style={{
              background: '#fff',
              border: `1.5px solid ${MELON}25`,
              boxShadow: `0 2px 10px ${MELON}08`,
          ...
          <span className="ml-auto text-xs font-semibold" style={{ color: '#A8836C' }}>
        ...
        <div
          ...
          style={{
            background: '#fff',
            border: `1.5px solid ${MELON}1F`,
            boxShadow: `0 4px 20px ${MELON}0D`,
          }}
        >
          {isLoading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="font-semibold" style={{ color: '#1C0A02' }}>
            ...
            <div className="divide-y" style={{ borderColor: `${MELON}10` }}>
              {filtered.map((n, i) => {
                ...
                const canRetry =
                  isFailed && (n.retryCount === undefined || n.retryCount < 3);
                    ...
                    <div className="flex w-full items-start gap-4 px-5 py-4 transition-colors hover:bg-orange-50/40">
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="mb-1 flex flex-wrap items-center gap-2">
                            ...
                            <span
                              ...
                              style={{
                                ...
                                color: channelCfg.tint,
                                border: `1px solid ${channelCfg.ring}`,
                              }}
                            >
                              {channelCfg.label}
                            </span>
                          ...
                          >
                            ...
                            {n.reservationId?.bookingRef && isExpanded && (
                              <>
                                <span>·</span>
                                <span className="font-mono">{n.reservationId.bookingRef}</span>
                              </>
                            )}
                            {n.externalId && isExpanded && (
                              <>
                                <span>·</span>
                      ...
                      {canRetry && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            retryMutation.mutate(n._id);
                          }}
                          disabled={retryMutation.isPending}
                          className="flex flex-shrink-0 items-center gap-1.5 self-center rounded-xl px-3 py-2 text-xs font-bold transition-all hover:scale-[1.03] disabled:opacity-50"
                          style={{
                            background: GRADIENT_SOFT,
                            color: CORAL,
                            border: `1.5px solid ${MELON}33`,
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: active ? 'none' : `1.5px solid ${MELON}25`,
