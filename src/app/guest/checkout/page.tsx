/**
 * src/app/guest/checkout/page.tsx  (Next.js App Router)
 * ─────────────────────────────────────────────────────────────────────────────
 * Express checkout page — opened when guest taps "✅ Express Checkout"
 * in the overdue card. Token in query param authenticates the session.
 *
 * Flow:
 *  1. Validate token → load reservation details
 *  2. Show summary (room, dates, outstanding balance)
 *  3. Guest confirms → POST /api/chatbot/guest/checkout  (new endpoint below)
    ...
    <main style={{ minHeight: '100vh', background: theme.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 420, background: theme.card, borderRadius: 32, border: `1.5px solid ${theme.border}`, overflow: 'hidden', boxShadow: '0 20px 60px rgba(225,117,92,0.14)' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #FF8B5E, #EC5B94)', padding: '24px 24px 20px', color: '#fff' }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.2em', opacity: 0.8, textTransform: 'uppercase' }}>Express Checkout</p>
        ...
        <div style={{ padding: 24 }}>
          ...
          {/* Loading */}
          {step === 'loading' && (
            <p style={{ textAlign: 'center', color: theme.muted, fontSize: 14 }}>Loading your reservation…</p>
          )}
          ...
          {step === 'review' && data && (
            <>
              ...
              {data.pendingAmount > 0 && (
                <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 16, padding: '12px 16px', marginBottom: 20, fontSize: 13, color: theme.red }}>
                  ⚠️ You have an outstanding balance of <strong>₹{data.pendingAmount.toLocaleString('en-IN')}</strong>. Please settle at the front desk or we will add it to your card on file.
                </div>
              )}

              <button onClick={confirmCheckout} style={{ width: '100%', background: 'linear-gradient(135deg, #1F7A5A, #16A34A)', color: '#fff', border: 'none', borderRadius: 20, padding: '14px 0', fontSize: 15, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 22px rgba(31,122,90,0.28)' }}>
          ...
          {/* Submitting */}
          {step === 'submitting' && (
            <p style={{ textAlign: 'center', color: theme.muted, fontSize: 14 }}>Processing your checkout…</p>
          )}
          ...
          {step === 'done' && (
            <div style={{ textAlign: 'center', paddingBottom: 8 }}>
              ...
              <p style={{ marginTop: 8, fontSize: 14, color: theme.muted }}>Thank you for staying with us. We hope to see you again soon!</p>
            </div>
          )}
